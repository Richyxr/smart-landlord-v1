...
export class StatementIngestionService {
  /**
   * Detects the file format from filename extension.
   */
  ...
  static detectProvider(text, filename) {
    ...
    // M-Pesa keywords
    if (/m-pesa|completion time|receipt no|paid in|withdrawn|pay bill|till number/i.test(content)) scores.MPESA += 20;
    if (content.includes('safaricom')) scores.MPESA += 10;
    if (content.includes('mpesa')) scores.MPESA += 15;

    // Loop keywords
    if (/loop ref|via ncba|wltbnk|customer number/i.test(content)) scores.LOOP_STATEMENT += 30;
    if (content.includes('account statement') && content.includes('loop')) scores.LOOP_STATEMENT += 15;

    // Co-op
    if (/co-operative bank|coop bank|statement of account|kcookena/i.test(content)) scores.COOP += 30;

    // KCB
    if (/kenya commercial bank|kcb/i.test(content)) scores.KCB += 30;

    // Equity
    if (/equity bank|equity/i.test(content)) scores.EQUITY += 25;

    // Absa
    if (/absa|barclays/i.test(content)) scores.ABSA += 30;

    // Generic Bank indicators
    if (/value date|transaction date|particulars|description|debit|credit|balance/i.test(content)) scores.GENERIC_BANK += 10;
    ...
    // Default to GENERIC_BANK if we have some general bank headers but no specific match
    if (bestProvider === 'UNKNOWN' && scores.GENERIC_BANK > 5) {
      bestProvider = 'GENERIC_BANK';
    }
  ...
  static async preview(buffer, filename, orgId, activeDb) {
    const sourceFormat = this.detectFileType(filename);
    ...
    if (sourceFormat === 'DOC') {
      return {
        ...
        preview_rows: [],
        safety_message: 'Preview only. No invoice, tenant balance, transaction, receipt, or ledger record has been changed.'
      };
    }
    ...
    try {
      if (sourceFormat === 'CSV') {
        ...
        const narrationIdx = headers.findIndex(h => /narration|desc/i.test(h));
        ...
        for (const line of dataLines) {
          ...
          const amountVal = parseFloat(String(rawAmount).replace(/,/g, ''));
        ...
        // Convert sheet to json objects with headers
        const rows = XLSX.utils.sheet_to_json(sheet, { defval: '' });
        
        let index = 1;
        for (const row of rows) {
          // Normalize column key access
          ...
          const amountVal = parseFloat(String(rawAmount || 0).replace(/,/g, ''));
        ...
        if (!rawText || rawText.trim().length < 15) {
          parserStatus = 'scanned_pdf_needs_ocr';
        } else {
          // Detect provider
          ...
          if (provider === 'LOOP_STATEMENT') {
            // NCBA Loop Parsing
            // Re-use current Loop parsing logic if defined, otherwise generic text parse
            // For safety, we can run text-line parser specifically tuned to Loop
            extractedRows = this.parseLoopPdfText(rawText);
          } else if (provider === 'MPESA') {
            // Safaricom Mpesa Parsing
    ...
    // Detect provider globally if not already set
    ...
    // Load DB records for duplicate checking and suggestions
    ...
    const existingHashes = new Set(existingEvidence.map(e => e.row_hash));
    ...
    for (const row of extractedRows) {
      // 1. Calculate row hash
      const rowHash = row.row_hash || this.generateRowHash(row);
      row.row_hash = rowHash;

      // 2. Classify Ignored Row using expanded keywords logic
      ...
      if (classified.status === 'ignored') {
        ...
        row.ignored_reason = classified.ignored_reason;
        ignoredCount++;
      } else {
        // 3. Duplicate Detection
        const isDuplicateHash = existingHashes.has(rowHash) || seenHashesInUpload.has(rowHash);
        const isDuplicateCode = row.transaction_code && (existingCodes.has(row.transaction_code) || seenCodesInUpload.has(row.transaction_code));

        if (isDuplicateHash || isDuplicateCode) {
          row.row_status = 'duplicate';
          row.duplicate_reason = isDuplicateHash ? 'Duplicate row structure already processed' : 'Transaction reference code already processed';
          duplicateCount++;
        } else {
          // Track unique references
          seenHashesInUpload.add(rowHash);
          if (row.transaction_code) {
            seenCodesInUpload.add(row.transaction_code);
          }
      ...
      // Generate suggested matches for ready rows
      ...
      // Map row keys for strict universal output contract compatibility
      normalizedRows.push({
  ...
  static parseUnstructuredText(text, format, provider) {
    ...
    for (const line of lines) {
      // Ignore general report headers
      if (/(balance b\/f|brought forward|closing balance|statement period)/i.test(line)) continue;

      // Scan for YYYY-MM-DD or DD/MM/YYYY or DD-MM-YYYY
      const dateMatch = line.match(/\b(\d{4}-\d{2}-\d{2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4})\b/);
      if (!dateMatch) continue;

      // Clean the line of the date to prevent year numbers matching as amounts
      const cleanLine = line.replace(dateMatch[0], ' ');

      // Scan for monetary amount
      ...
      // Scan for potential transaction code
  ...
  static parseLoopPdfText(text) {
    ...
    for (const line of lines) {
      // Loop statements usually have a transaction code (e.g. alphanumeric) and date
