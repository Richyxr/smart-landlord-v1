export class PaymentDomainService {
  ...
  async allocatePayment(input) {
    ...
    const payment = await this.db.findOne('payments', { id: Number(payment_id), organization_id: Number(organization_id) });
    if (!payment) {
      throw new Error('Payment not found.');
    }

    const invoice = await this.db.findOne('invoices', { id: Number(invoice_id), organization_id: Number(organization_id) });
    if (!invoice) {
    ...
    const existing = await this.db.findOne('payment_allocations', filter);
    if (existing) {
      return existing;
    }

    // Check payment capacity limit: sum(allocations) + new_amount <= payment.amount
    const allocations = await this.db.find('payment_allocations', { organization_id: Number(organization_id), payment_id: Number(payment_id) }) || [];
    const totalAllocated = allocations.reduce((sum, a) => sum + Number(a.amount || a.amount_allocated || 0), 0);
    if (totalAllocated + Number(amount) > Number(payment.amount)) {
      throw new Error('Allocation total exceeds payment amount.');
    }

    // Insert allocation
    const newAllocation = await this.db.insert('payment_allocations', {
      organization_id: Number(organization_id),
      payment_id: Number(payment_id),
      invoice_id: Number(invoice_id),
      transaction_id: payment.source_type === 'manual' ? payment.source_id : null, // bridge support
      amount: Number(amount),
      amount_allocated: Number(amount), // legacy support
    ...
    // Update payment's allocation status
    ...
    await this.db.update('payments', payment.id, { allocation_status: newAllocStatus });
